...
class CF7GSC_googlesheet
{
    ...
    public static function preauth($access_code)
    {
        ...
        if (is_multisite()) {
            // Fetch API creds
            $api_creds = get_site_option('cf7gsc_free_api_creds');
        } else {
            // Fetch API creds
            $api_creds = get_option('cf7gsc_free_api_creds');
        }

        $newClientSecret = get_option('is_new_client_secret_cf7gscfree');
    ...
    public static function updateToken($tokenData)
    {
        ...
        try {
            ...
            Gs_Connector_Free_Utility::gs_debug_log("Token write fail! - " . $e->getMessage());
        }
    }
...
if ( isset( $_POST['gf-custom-header'] ) ) {
    ...
    public function gsheet_get_google_account()
    {
        ...
        try {
            ...
            Gs_Connector_Free_Utility::gs_debug_log(__METHOD__ . " Error in fetching user info: \n " . $e->getMessage());
            return false;
        }
